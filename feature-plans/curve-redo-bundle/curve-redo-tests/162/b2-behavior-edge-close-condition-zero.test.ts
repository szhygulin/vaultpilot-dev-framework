...
test("the close condition (count to 0) is referenced", () => {
  // either explicit close-when-0 phrasing, or at minimum mention of advisory count clearing
  expect(nearAdvisory(docs())).toMatch(/(drops? to 0|reaches? 0|count.*0|close.*0|zero advisor)/i);
});
