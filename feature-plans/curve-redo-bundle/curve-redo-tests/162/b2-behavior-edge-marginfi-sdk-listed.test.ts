...
function nearAdvisory(text: string): string {
  // Return the union of windows around every GHSA / bigint-buffer mention
...
test("MarginFi appears within the advisory tracking context", () => {
  const ctx = nearAdvisory(docs());
  expect(ctx.length).toBeGreaterThan(0);
  expect(ctx).toMatch(/marginfi|mrgnlabs/i);
});
