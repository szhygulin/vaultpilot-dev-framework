...
test("advisory tracking doc clarifies the input source is RPC/account data, not user HTTP input", () => {
  const text = collectAdvisoryDocs();
  // Either explicitly says "not user input" / similar, OR explains the input vector is RPC/account-data.
  expect(text.toLowerCase()).toMatch(/not\s+a?\s*user[- ]input|account\s+data|rpc\s+response/);
});
