...
test("docs explain why dropping Solana SDKs is not viable", () => {
  const blob = collectDocs();
  expect(blob.toLowerCase()).toMatch(/(can'?t|cannot|won'?t)\s+(drop|remove)|protocol coverage|loss(ing)? .*coverage|breaking.*solana/);
});
