...
test("formatTightenProposal: returns a non-empty string", () => {
  const md = fakeMd([
    { run: "run-1", issue: 137, heading: "H", body: "Original body content here." },
  ]);
  ...
  assert.ok(out.length > 0, "expected non-empty formatter output");
});
