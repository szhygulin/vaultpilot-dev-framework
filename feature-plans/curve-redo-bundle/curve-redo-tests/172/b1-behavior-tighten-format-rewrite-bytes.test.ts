...
test("formatTightenProposal: per-section block has Rewrite: <N> bytes", () => {
  const md = fakeMd([
    { run: "run-1", issue: 137, heading: "H", body: "Long original prose body to tighten." },
  ]);
  const sections = parseClaudeMdSections(md);
  const proposal: TightenProposal = {
    agentId: "agent-test",
    rewrites: [{ sectionId: "s0", rewrittenBody: "Short.", estimatedBytesSaved: 30 }],
    unchangedSectionIds: [],
    estimatedBytesSaved: 30,
    warnings: [],
  };
  const out = formatTightenProposal(proposal, sections);
  assert.match(out, /Rewrite:\s*\d+\s*bytes/);
});
