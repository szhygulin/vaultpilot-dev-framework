...
test("findTightenWarnings: single date preserved across rewrite -> no warnings", () => {
  const md = fakeMd([{ run: "run-A", issue: 100, heading: "Rule A", body: "Past incident 2026-04-28: foo happened during deploy." }]);
  const sections = parseClaudeMdSections(md);
  const proposal: TightenProposal = {
    agentId: "agent-x",
    rewrites: [{ sectionId: "s0", rewrittenBody: "Incident 2026-04-28: foo.", estimatedBytesSaved: 30 }],
    unchangedSectionIds: [],
