...
test("findTightenWarnings: rewrite preserves single #137 xref -> no warnings", () => {
  const md = fakeMd([{ run: "run-A", issue: 100, heading: "Rule A", body: "Rule predates issue #137 and its derivatives." }]);
  const sections = parseClaudeMdSections(md);
  const proposal: TightenProposal = {
    agentId: "agent-x",
    rewrites: [{ sectionId: "s0", rewrittenBody: "From #137.", estimatedBytesSaved: 30 }],
    unchangedSectionIds: [],
