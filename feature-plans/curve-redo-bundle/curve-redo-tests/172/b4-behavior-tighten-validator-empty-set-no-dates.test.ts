...
test("findDroppedInvariants: source with no dates yields no warnings on rewrite", () => {
  const md = fakeMd([
    { run: "run-A", issue: 100, heading: "Rule A", body: "Generic guidance with no dates, no xrefs, no thresholds." },
  ]);
      ...
      rewrites: [
        {
          ...
          rewrittenBody: "Brief generic guidance.",
