...
let _c = 0;
const uniq = () => `t-rr-last-${process.pid}-${Date.now()}-${++_c}`;

test("recordReinforcement sets lastReinforcedAt on the cited section", async () => {
  const agentId = uniq();
  ...
  try {
    await recordReinforcement({ agentId, runId: "run-ts", citedSectionStableIds: ["sid-x"] });
    const data = JSON.parse(await fs.readFile(statePath(agentId), "utf8"));
    const sec = data.sections.find((s: any) => s.sectionId === "sid-x");
    assert.ok(typeof sec.lastReinforcedAt === "string" && sec.lastReinforcedAt.length > 0,
      "lastReinforcedAt must be set to a non-empty string");
  } finally {
