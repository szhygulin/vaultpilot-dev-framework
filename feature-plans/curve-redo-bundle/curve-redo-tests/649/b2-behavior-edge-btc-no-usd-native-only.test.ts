...
describe("Edge: BTC native-only fallback when USD unavailable", () => {
  it("render with no usd produces output mentioning BTC but no $", async () => {
    const out = await tryRender({ feeNative: "0.0001", chain: "BTC" });
