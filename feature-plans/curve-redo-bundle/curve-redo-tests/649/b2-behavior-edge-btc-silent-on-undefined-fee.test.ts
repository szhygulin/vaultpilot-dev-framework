...
describe("Edge: BTC cost preview silent on undefined fee", () => {
  it("undefined feeNative for BTC produces no preview block", async () => {
    const positive = await tryRender({ feeNative: "0.0001", chain: "BTC", usd: 60000 });
    expect(positive).toBeDefined();
    expect(typeof positive).toBe("string");
    expect(String(positive).toUpperCase()).toContain("BTC");

    const silent = await tryRender({ feeNative: undefined, chain: "BTC", usd: 60000 });
