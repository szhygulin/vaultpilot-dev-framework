...
test("BTC: negative feeNative is not rendered as a fee", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "btc", kind: "btc-send", to: "bc1q", amount: "0.001", feeNative: "-0.001" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/-0\.001/);
