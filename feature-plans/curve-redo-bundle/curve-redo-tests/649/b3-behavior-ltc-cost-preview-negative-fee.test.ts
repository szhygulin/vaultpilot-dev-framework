...
test("LTC: negative feeNative is not rendered as a fee", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "litecoin", kind: "ltc-send", to: "ltc1", amount: "0.5", feeNative: "-0.5" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/-0\.5/);

  const valid: any = { ...tx, feeNative: "0.0005" };
