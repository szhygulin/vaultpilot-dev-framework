...
test("Solana: negative feeNative is rejected, valid feeNative is rendered", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "solana", kind: "solana-transfer", to: "def", amount: "1", feeNative: "-0.0001" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/-0\.0001/);
