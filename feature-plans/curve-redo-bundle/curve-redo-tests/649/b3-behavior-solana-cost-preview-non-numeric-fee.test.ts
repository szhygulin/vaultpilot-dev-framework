...
test("Solana: garbage feeNative does not appear in output", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "solana", kind: "solana-transfer", to: "def", amount: "1", feeNative: "abcxyz" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/abcxyz/);
  expect(asText(r1)).not.toMatch(/NaN/);

  const valid: any = { ...tx, feeNative: "0.00001" };
