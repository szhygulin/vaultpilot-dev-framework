...
test("Solana modules assign a fee-string field on the unsigned-tx envelope", () => {
  const src = readAllTs(resolve(repoRoot, "src/modules/solana"));
  expect(src.length).toBeGreaterThan(0);
  expect(src).toMatch(/\bfee(?:Native|Sol|Solana|Lamports|Amount|Total|Str|String)\b\s*[:=]/);
});
