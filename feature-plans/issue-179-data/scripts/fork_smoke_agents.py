...
shutil.copy(REG, REG + '.bak.smoke179')
now = '2026-05-06T05:00:00.000Z'

meta = json.load(open('/tmp/smoke_trim_meta.json'))
existing = {a['agentId'] for a in reg['agents']}
for m in meta:
    aid = m['aid']
    if aid in existing:
        # remove old + replace (idempotent re-run)
    ...
    print(f'Forked {aid} ("Smoke Study {m["kb"]}KB") -> {dest}/CLAUDE.md ({m["bytes"]}B)')
