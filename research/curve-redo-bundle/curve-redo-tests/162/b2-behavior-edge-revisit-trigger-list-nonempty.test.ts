...
test("at least one concrete revisit trigger is named", () => {
  const ctx = nearAdvisory(docs());
  // Either patched/fix-released, or migration off bigint-buffer, or readBigUInt64LE alternative.
  expect(ctx).toMatch(/(patched|fix(ed)? version|migrat(e|ing|ion)|readBigUInt64LE|drops? bigint-buffer|moves? off)/i);
});
