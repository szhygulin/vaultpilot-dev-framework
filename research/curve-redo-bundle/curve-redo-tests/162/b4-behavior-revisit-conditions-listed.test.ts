...
test("docs list revisit conditions for the advisory", () => {
  const blob = collectDocs();
  expect(blob.toLowerCase()).toMatch(/revisit|reevaluate|re-evaluate|monitor|recheck|re-check|when.*publish/);
});
