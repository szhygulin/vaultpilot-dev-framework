...
test("formatTightenProposal: per-section block surfaces the section heading", () => {
  const md = fakeMd([
    { run: "run-1", issue: 137, heading: "DistinctiveHeadingZQX", body: "Original body that gets tightened down." },
  ]);
  ...
  assert.ok(out.includes("DistinctiveHeadingZQX"), "expected heading text in output");
});
