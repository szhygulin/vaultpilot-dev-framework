...
test("formatTightenProposal: per-section block reports savings in bytes", () => {
  const md = fakeMd([
    { run: "run-1", issue: 137, heading: "H", body: "A long original body that becomes much shorter." },
  ]);
  const sections = parseClaudeMdSections(md);
  const proposal: TightenProposal = {
    agentId: "agent-test",
    rewrites: [{ sectionId: "s0", rewrittenBody: "Short.", estimatedBytesSaved: 40 }],
    unchangedSectionIds: [],
    estimatedBytesSaved: 40,
    warnings: [],
  };
  const out = formatTightenProposal(proposal, sections);
  assert.match(out, /savings/i);
  assert.match(out, /\d+\s*bytes/);
});
