...
test("findTightenWarnings: rewrittenBody identical to source -> no warnings", () => {
  const body = "Past incident 2026-04-28: see #137. Capped at 5KB.";
  const md = fakeMd([{ run: "run-A", issue: 100, heading: "Rule A", body }]);
  const sections = parseClaudeMdSections(md);
  const proposal: TightenProposal = {
    agentId: "agent-x",
    rewrites: [{ sectionId: "s0", rewrittenBody: body, estimatedBytesSaved: 0 }],
    unchangedSectionIds: [],
