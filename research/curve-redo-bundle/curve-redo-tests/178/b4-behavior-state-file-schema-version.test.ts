...
let _c = 0;
const uniq = () => `t-schver-${process.pid}-${Date.now()}-${++_c}`;

test("state file schemaVersion equals 1 after recordReinforcement", async () => {
  const agentId = uniq();
  ...
  await fs.writeFile(statePath(agentId), JSON.stringify({
    ...
    schemaVersion: 1,
    sections: [{
      sectionId: "sid-s", introducedRunId: "run-init", introducedAt: "2026-05-01T00:00:00.000Z",
      reinforcementRuns: [], pushbackRuns: [], pastIncidentDates: [], crossReferenceCount: 0,
    }],
  }));
  try {
    await recordReinforcement({ agentId, runId: "r1", citedSectionStableIds: ["sid-s"] });
