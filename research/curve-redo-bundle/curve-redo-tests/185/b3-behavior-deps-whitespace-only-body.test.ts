...
function getParser(): (body: string) => unknown {
  ...
  throw new Error("No dependency parser export. Available: " + Object.keys(depsModule).join(", "));
}
...
test("whitespace-only body yields no extracted dependency refs", () => {
  const parse = getParser();
  assert.deepEqual(nums(parse("   \n\n\t   \n")), []);
});
