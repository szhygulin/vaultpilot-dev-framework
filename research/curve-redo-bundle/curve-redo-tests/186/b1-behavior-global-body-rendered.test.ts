...
test("body of ~/.claude/CLAUDE.md is rendered verbatim in the prompt", async () => {
  ...
  const sentinel = "GLOBAL_BODY_SENTINEL_8472alpha";
  ...
  fs.writeFileSync(path.join(agentsDir, agentId, "CLAUDE.md"), "## Other\n");
  ...
  try {
    ...
    assert.ok(result.includes(sentinel), "expected global body sentinel in prompt");
  } finally {
