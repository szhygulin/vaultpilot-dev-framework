...
test("global CLAUDE.md present: prompt includes 'User global CLAUDE.md' header", async () => {
  ...
  fs.writeFileSync(path.join(home, ".claude", "CLAUDE.md"), "## Operator Rule\nbe nice\n");
  fs.mkdirSync(path.join(agentsDir, agentId), { recursive: true });
  fs.writeFileSync(path.join(agentsDir, agentId, "CLAUDE.md"), "## Per Agent\nfocus\n");
  ...
  try {
    ...
    assert.match(result, /User global CLAUDE\.md/);
  } finally {
