...
describe("Edge: LTC smallest unit (1 litoshi)", () => {
  it("renders smallest unit without truncating to zero", async () => {
    const out = await tryRender({ feeNative: "0.00000001", chain: "LTC" });
    expect(out).toBeDefined();
    const s = String(out);
    expect(s.toUpperCase()).toContain("LTC");
    expect(s).toMatch(/0\.0{0,7}1|1e-8/);
  });
});
