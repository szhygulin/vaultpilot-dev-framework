...
describe("Edge: TRON native-only fallback when USD unavailable", () => {
  it("render with no usd mentions TRX but no $", async () => {
    const out = await tryRender({ feeNative: "5", chain: "TRX" });
