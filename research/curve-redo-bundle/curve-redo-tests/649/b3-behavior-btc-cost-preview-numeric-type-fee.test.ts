...
test("BTC: numeric feeNative does not crash the renderer", async () => {
  vi.resetModules();
  ...
  // Per the issue contract feeNative is a string. Passing a number violates the contract;
  // the renderer must not throw and must not produce '[object Object]' / 'NaN' artifacts.
  const tx: any = { chain: "btc", kind: "btc-send", to: "bc1q", amount: "0.001", feeNative: 0.0001 };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/\[object Object\]/);
