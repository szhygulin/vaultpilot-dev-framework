...
test("LTC: non-numeric feeNative does not appear in output", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "litecoin", kind: "ltc-send", to: "ltc1", amount: "0.5", feeNative: "junk-value" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/junk-value/);
  expect(asText(r1)).not.toMatch(/NaN/);

  const valid: any = { ...tx, feeNative: "0.0003" };
