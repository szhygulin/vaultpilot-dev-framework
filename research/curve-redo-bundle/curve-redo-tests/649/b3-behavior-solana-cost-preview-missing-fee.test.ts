...
test("Solana: missing feeNative is silent, valid feeNative renders SOL", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "solana", kind: "solana-transfer", from: "abc", to: "def", amount: "1", asset: "SOL" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/network fee|estimated/i);

  const valid: any = { ...tx, feeNative: "0.000005" };
