...
test("TRON: non-numeric feeNative does not produce a NaN preview", async () => {
  vi.resetModules();
  ...
  const tx: any = { chain: "tron", kind: "tron-trx-transfer", to: "TLs", amount: "1", feeNative: "not-a-number" };
  let err: unknown; let r1: any;
  try { r1 = await render(tx); } catch (e) { err = e; }
  expect(err).toBeUndefined();
  expect(asText(r1)).not.toMatch(/NaN/);
  expect(asText(r1)).not.toMatch(/not-a-number/);

  const valid: any = { ...tx, feeNative: "1.234" };
