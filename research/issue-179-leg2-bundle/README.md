...
```bash
cd $REPO
mkdir -p agents/agent-916a research/issue-179-data/trims-phase3 research/issue-179-data/logs-mcp
...
cp research/issue-179-leg2-bundle/trims-phase3/*.md research/issue-179-data/trims-phase3/
cp research/issue-179-leg2-bundle/agents-spec-phase3*.json research/issue-179-data/
...
cp research/issue-179-leg2-bundle/leg-1-curve-study-mcp.json research/issue-179-data/curve-study-mcp.json
```
...
jq -r '.[].clonePath' research/issue-179-data/agents-spec-phase3-dev.json > /tmp/leg2-clone-paths.txt
...
```bash
mkdir -p research/issue-179-data/logs-dev
...
VP_DEV_MAX_COST_USD=15 node dist/bin/vp-dev.js research curve-study \
  ...
  --logs-dir research/issue-179-data/logs-dev \
  --output research/issue-179-data/curve-study-dev.json \
...
```bash
jq '[.[0]]' research/issue-179-data/agents-spec-phase3-dev.json > /tmp/agents-spec-smoke.json
...
VP_DEV_MAX_COST_USD=15 node dist/bin/vp-dev.js research curve-study \
  ...
  --logs-dir research/issue-179-data/logs-smoke-leg2 \
  --output research/issue-179-data/curve-study-smoke-leg2.json \
  --curve-form linear-log
```
...
ls research/issue-179-data/logs-dev/curveStudy-*.log | wc -l   # expect 126

# Combined fit: per-agent cells pool across both legs, scored together, fit linear-log on the 18 sample points
node research/issue-179-leg2-bundle/combine-legs.cjs \
  research/issue-179-data/logs-mcp \
  research/issue-179-data/logs-dev \
  research/issue-179-data/agents-spec-phase3-dev.json \
  research/issue-179-data/curve-study-combined-k13.json
```
